...
function Vote() {
    ...
    useEffect(()=>{
            const getelec=async()=>{
              ...
              try{
                  ...
                  console.log("fhvbnb",err.message);
                  setisloading(false)
  ...
  return (
    <div className='votepage'>
        ...
        {voter && <div className='voterdetailss'>
            <h2>Voter Details</h2>
            <p><b>VoterAccount:</b> {details.account}</p>
            <p><b>Votername:</b> {voter?.votername}</p>
            <p><b>Status:</b> {voter.authorized ? "Authorized" : "NotAuthorized"}</p>
        </div>}
        ...
        {elections?.length>0 ? elections.map((election,ind)=>(
            ...
            <button type='button' className='votebtn'>Vote</button>
            </NavLink> 
            </div>
